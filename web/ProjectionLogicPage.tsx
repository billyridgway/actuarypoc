                    ...
                    {row.calculation && <tr className="projection-ledger__calculation"><td colSpan={12}>
                      <details>
                        ...
                        <div className="calculation-audit">
                          ...
                          {!!row.calculation.months?.length && <details><summary>Show 12 monthly calculations</summary><div className="table-scroll"><table><thead><tr><th>Month</th><th>Premium</th><th>NAR used for COI</th><th>COI equation</th><th>COI charge</th><th>Fee</th><th>Interest</th><th>Ending value</th></tr></thead><tbody>{row.calculation.months.map((month) => {
                            const divisor = Number(month.coiDivisor || 1);
                            const annualizationDivisor = Number(month.coiAnnualizationDivisor || 1);
                            return <tr key={month.month}><td>{month.month}</td><td>{formatCurrency(month.premium)}</td><td>{formatCurrency(month.netAmountAtRisk)}</td><td className="calculation-equation"><span className="calculation-equation__basis">{month.coiBasisLabel || "COI basis"}</span>{formatCurrency(month.coiBasis)}{divisor !== 1 ? ` ÷ ${divisor.toLocaleString("en-US")}` : ""} × {month.coiRate}{annualizationDivisor !== 1 ? ` ÷ ${annualizationDivisor}` : ""} = <strong>{formatCurrency(month.coiCharge)}</strong><small>{month.coiRateUnit}</small></td><td>{formatCurrency(month.coiCharge)}</td><td>{formatCurrency(month.policyFee)}</td><td>{formatCurrency(month.interest)}</td><td>{formatCurrency(month.endingPolicyValue)}</td></tr>;
