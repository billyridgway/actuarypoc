...
def _pmr_system_prompt() -> str:
    return (
        ...
        "projection summaries, assumptions, known gaps, prior "
        "decisions, and (for some products) mechanics-informed "
        "assumption discovery and Product Mechanics metadata. In "
        "particular:\n"
        "- The 'assumptions' block may contain a 'mechanicsInformed' "
        "sub-object with fields like status, assumptionSetId, and "
        "mechanicAssumptions (non-executable, text-only assumptions "
        "anchored to Product Mechanics).\n"
        "- The payload may include 'productMechanics' and related "
        "validation / patch-preview blocks describing how filings map "
        "to DSL.\n"
        "- Some PMRs may include additional advisory fields such as "
        "productUnderstandingGaps or similar structures that describe "
        "conceptual gaps in product understanding.\n\n"
        ...
        "- If mechanics-informed assumptions exist (for example, "
        "  assumptions.mechanicsInformed.status is not 'not_started' "
        "  and mechanicsInformed.mechanicAssumptions is non-empty), do "
        "  NOT claim that there are 'no assumptions filed or proposed'. "
        "  Instead, explain that mechanics-informed assumptions have "
        "  been extracted from filings but are not yet executable DSL "
        "  when that is the case.\n"
        "- Only say that 'no scenarios were provided' when the PMR "
        "  truly contains no scenarios. If scenarios are present but "
        "  lack full projection evidence, explain that limitation "
        "  rather than claiming scenarios are absent."
    )
...
def _decision_system_prompt() -> str:
    return (
        ...
        "There are TWO distinct questions you must keep separate:\n"
        "1) Is PRODUCT UNDERSTANDING sufficiently complete? (Do we "
        "   understand the mechanics, assumptions, and scenarios well "
        "   enough to proceed?)\n"
        "2) Is MODEL IMPLEMENTATION complete enough for production "
        "   projections? (Is executable DSL / a projection engine in "
        "   place and validated?)\n\n"
        "For generic products that have mechanics-informed assumptions "
        "but no executable DSL yet (for example, a populated \"assumptions"
        ".mechanicsInformed\" block but no working projection model), "
        "you should primarily answer the question: \"Can we proceed to DSL "
        "authoring?\" Missing DSL fragments should be treated as clear, "
        "actionable future work rather than an automatic PMR failure.\n\n"
        "In your rationale, explicitly distinguish between:\n"
        "- Product understanding completeness (filings reviewed, "
        "  mechanics and mechanicsInformed assumptions, scenarios, known "
        "  gaps).\n"
        "- Model implementation completeness (presence/absence of DSL, "
        "  projection logic, and validation).\n"
        "If product understanding is sufficient to start DSL authoring "
        "but the model implementation is still missing, it is acceptable "
        "to recommend proceeding with DSL work while clearly flagging "
        "that production projections are not yet possible.\n\n"
        "Your output MUST be a single JSON object with this shape (no "
        "extra commentary):\n"
        "{\n"
        "  \"suggested_decision\": string,    // e.g. 'proceed_to_dsl_authoring', 'proceed_with_caveats', 'request_more_understanding', 'defer'\n"
        "  \"suggested_exclusions\": string | null,\n"
        "  \"rationale\": [string, ...],     // include separate bullets for product-understanding vs model-implementation status\n"
        "  \"risk_status\": string | null     // e.g. 'understanding_ready_model_missing', 'understanding_gaps', 'high_risk'\n"
        "}\n\n"
        "If you are unsure, prefer a conservative recommendation such as "
        "'request_more_understanding' or 'proceed_with_caveats' and "
        "explain exactly what additional evidence or checks are needed."
    )
